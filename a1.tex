...
% "If a group contains infinitely many people, then it also contains 40 people who do not know each other."
...
% Transform the $\oor$ connection to $\iimplies$,

% \begin{align*}
% \exists n \in \mathbb{N}. & \forall G \in \mathcal{P}(U). \exists H \in \mathcal{P}(U). \forall x \in H. \forall y \in H.\\
% & (size(G)  \geq n) \iimplies [(size(H) = 40) \aand \\ & (\nnot(K(x,y)) \oor (x=y)) \aand \\ & (In(x ,G))].
% \end{align*}

% Then move $\exists n\in\mathbb{N}$ within the hypothesis of $\text{ IMPLIES }$,

% \begin{align*}
% \forall G \in \mathcal{P}(U). & \exists H \in \mathcal{P}(U). \forall x \in H. \forall y \in H.\\
% & (\forall n \in \mathbb{N}. size(G) \geq n) \iimplies [(size(H) = 40) \aand \\ & (\nnot(K(x,y)) \oor (x=y)) \aand \\ & (In(x ,G))].
% \end{align*}

% Now we can see the hypothesis is "the size of a group of people G is infinite", and the conclusion is "G contains 40 people who don't know each other", this is logically equivalent to "either the size of a group of people is finite, or it contains 40 people who don't know each other."
